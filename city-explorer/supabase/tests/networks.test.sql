...
insert into public.stations (id, city_id, external_source, external_id, name, lat, lon, modes, network) values
  ('30000000-0000-0000-0000-0000000000b1', '00000000-0000-0000-0000-0000000000bb', 'test', 'r1', 'Rail A', 51.500, -0.140, '{tube}', 'rail'),
  ('30000000-0000-0000-0000-0000000000b2', '00000000-0000-0000-0000-0000000000bb', 'test', 'r2', 'Rail B', 51.510, -0.140, '{tube}', 'rail'),
  ('30000000-0000-0000-0000-0000000000b3', '00000000-0000-0000-0000-0000000000bb', 'test', 's1', 'Stop 1', 51.520, -0.140, '{bus}',  'bus'),
  ('30000000-0000-0000-0000-0000000000b4', '00000000-0000-0000-0000-0000000000bb', 'test', 's2', 'Stop 2', 51.530, -0.140, '{bus}',  'bus');
...
select lives_ok($$ select public.check_in('30000000-0000-0000-0000-0000000000b3', 51.520, -0.140, 10) $$, 'stop 1');
reset role; update public.visits set visited_at = visited_at - interval '30 minutes'; set local role authenticated;

select is(
  (select r->'new_achievements' from public.check_in('30000000-0000-0000-0000-0000000000b4', 51.530, -0.140, 10) r),
...
select is(
  (select name from public.nearest_stations(51.5195, -0.140, 1, 'rail')),
...
reset role; update public.visits set visited_at = visited_at - interval '30 minutes'; set local role authenticated;
select lives_ok($$ select public.check_in('30000000-0000-0000-0000-0000000000b1', 51.500, -0.140, 10) $$, 'rail A');
reset role; update public.visits set visited_at = visited_at - interval '30 minutes'; set local role authenticated;
select is(
  (select r->'new_achievements' from public.check_in('30000000-0000-0000-0000-0000000000b2', 51.510, -0.140, 10) r),
